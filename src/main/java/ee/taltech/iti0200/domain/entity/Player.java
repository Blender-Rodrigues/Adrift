...
public class Player extends Living {
    ...
    public void useJetpack(long currentTick) {
        for (Object equip : consumables) {
            if (equip instanceof Jetpack) {
                setJumpsLeft(Math.min(1, getJumpsLeft()));
                ((Jetpack) equip).use(currentTick);
                break;
            }
        }
    }
